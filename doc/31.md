...
我们在前几次讲的`互斥锁`、`条件变量`和`原子操作`都是最基本重要的同步工具。在 Go 语言中，除了`通道`之外，它们也算是最为常用的`并发安全工具`了。
...
与sync.WaitGroup类型一样，`sync.Once`类型（以下简称Once类型）也属于结构体类型，同样也是`开箱即用和并发安全的`。由于这个类型中包含了一个sync.Mutex类型的字段，所以，复制该类型的值也会导致功能的失效。
